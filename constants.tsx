        ...
        learningSituations: [
          {
            ...
            context: 'A "UsiPrecision Componentes Mecânicos Ltda." enfrenta um aumento no índice de devoluções de um de seus principais clientes devido à inconsistência dimensional em lotes de "Pinos de Ancoragem". O gestor da Qualidade identificou que a causa raiz do problema pode estar na falta de um procedimento padronizado de medição e na variação de critérios entre os inspetores. A empresa decidiu investir na capacitação da equipe para garantir que 100% das peças enviadas estejam rigorosamente dentro das especificações.',
            challenge: 'Sua missão é realizar uma Inspeção dimensional completa em um lote de 10 "Pinos de Ancoragem". Vocês deverão: a) Analisar o Desenho Técnico; b) Planejar a Inspeção (selecionar instrumentos: escala, trena, paquímetro, micrômetro, goniômetro, relógios e verificadores); c) Executar as Medições aplicando as técnicas corretas; d) Registrar os Dados no Relatório de Inspeção Dimensional; e) Emitir o Parecer (Aprovado ou Reprovado) justificando a decisão.',
            expectedResults: [
              'Relatórios de Inspeção Dimensional: Preenchidos com medições, comparações com as especificações e parecer justificado.',
              'Demonstração Prática: Manuseio correto e seguro de todos os instrumentos propostos, realizando leituras precisas.',
              'Argumentação Técnica: Capacidade de explicar o processo, justificar a escolha dos instrumentos e defender as decisões tomadas.',
              'Organização do Posto de Trabalho: Bancada limpa e organizada, com instrumentos limpos e guardados em seus estojos.'
            ]
          }
        ],
        rubrics: [
