...
export const COMMON_SA_FUSI = [
  {
    ...
    context: 'A "AgroMaq Industrial", empresa especializada na fabricação de pequenos implementos agrícolas, enfrenta um problema crítico: a linha de produção de um de seus pulverizadores está parada devido à falta de um subconjunto de ajuste mecânico. O fornecedor original das peças (eixos, manípulos e luvas) encerrou suas atividades, e a compra de um novo lote de um fornecedor alternativo levaria semanas, causando prejuízos significativos. Para solucionar o problema de forma rápida e eficiente, a gerência decidiu internalizar a produção desses componentes. O departamento de engenharia disponibilizou os desenhos técnicos do conjunto e solicitou ao setor de usinagem a fabricação de um lote piloto para validação. Vocês, como a equipe de operadores de máquinas da "AgroMaq", receberam a missão de produzir as primeiras unidades deste conjunto. A tarefa exige não apenas habilidade na operação das máquinas, mas também a capacidade de interpretar os desenhos, calcular os parâmetros de corte corretos, executar as operações de torneamento, furação e rosqueamento com precisão, e garantir a qualidade final para que as peças possam ser montadas e funcionem perfeitamente.',
    challenge: 'Com base nos desenhos técnicos fornecidos e no material bruto, a equipe deverá executar um projeto completo de usinagem, focando nas operações de torneamento, para fabricar os componentes: a) Planejar e Calcular: Para cada peça a ser usinada (Eixo cilíndrico de quatro corpos, Eixo roscado, Manípulo, Luva com dois corpos internos), a equipe deverá calcular e registrar os parâmetros de corte essenciais (Velocidade de Corte, RPM, avanço) para cada operação (facear, tornear, chanfrar, furar, roscar), considerando as ferramentas e o material especificados. b) Executar e Controlar: Utilizando o torno mecânico convencional e a furadeira de bancada, a equipe deverá usinar as peças seguindo a sequência operacional planejada. Durante todo o processo, será necessário aplicar corretamente o fluido de refrigeração, zelar pela organização e segurança do ambiente, e realizar o controle dimensional contínuo com paquímetro e micrômetro, preenchendo uma ficha de autoinspeção. As operações incluem: No torno: faceamento, torneamento de múltiplos diâmetros, furação de centro, furação com broca helicoidal, corte de canais e rosqueamento com cossinete. Na furadeira/bancada: furação e rosqueamento manual com macho para o "Manípulo". c) Montar e Validar: Ao final da usinagem, a equipe deverá realizar a montagem do "Manípulo" no "Eixo roscado" para comprovar a funcionalidade e a precisão do conjunto roscado.',
    expectedResults: [
  ...
  },
  {
    ...
    context: 'A "Fresanatec Soluções Industriais", uma empresa metalmecânica com 30 funcionários, especializada na fabricação de peças e na prestação de serviços de manutenção, acaba de investir na ampliação de seu parque de máquinas com a aquisição de uma nova fresadora convencional. O objetivo é aumentar a capacidade produtiva e diversificar os serviços oferecidos aos seus clientes. Para que a nova máquina atinja seu potencial máximo, o gerente de produção, Sr. Almeida, identificou a necessidade de fabricar um conjunto de acessórios e dispositivos de fixação que não acompanharam o equipamento. Esses componentes são importantes para a versatilidade, segurança e eficiência das futuras operações. O conjunto inclui: um bloco fresado para referência, um bloco com rebaixo para exercícios de posicionamento, castanhas moles para fixação de peças delicadas, uma base para as castanhas, mordentes de proteção em alumínio e um protótipo de coletor de serragem para manter a área limpa e segura. Como a equipe de usinadores experientes está totalmente alocada em projetos de clientes, o Sr. Almeida decidiu transformar essa necessidade interna em uma oportunidade de desenvolvimento para a nova equipe de aprendizes.',
    challenge: 'Como equipe de aprendizes da Fresanatec, vocês têm o desafio de planejar, executar e controlar a fabricação do conjunto completo de seis componentes para a nova fresadora convencional. Para isso, a equipe deverá: a) Analisar criticamente os desenhos técnicos de cada uma das seis peças, interpretando todas as cotas, tolerâncias e especificações. b) Elaborar um plano de trabalho detalhado (folha de processo) para cada componente, definindo a sequência lógica das operações de usinagem (fresagem, furação, rosqueamento), selecionando as ferramentas de corte e de fixação adequadas, e calculando os parâmetros de corte (RPM, avanços) para cada operação. c) Aplicar as normas de saúde e segurança no trabalho, identificando os riscos envolvidos e utilizando corretamente os Equipamentos de Proteção Individual (EPIs) e Coletivos (EPCs). d) Executar as operações de fresagem de baixa complexidade (faceamento, rebaixos, superfícies paralelas e perpendiculares), furação e rosqueamento manual, seguindo o plano de trabalho estabelecido. e) Gerenciar o uso de fluidos de refrigeração e realizar o controle de qualidade dimensional e visual das peças.',
    expectedResults: [
      'a) O conjunto de 6 (seis) componentes mecânicos (Bloco fresado, Bloco rebaixado, Castanha mole, Base para castanha, Mordente de proteção, Coletor de serragem) usinados, finalizados e em total conformidade com os desenhos.',
      'b) O Plano de Trabalho (Folha de Processo) completo e devidamente preenchido para cada um dos componentes fabricados.',
      'c) Um Relatório de Controle de Qualidade para cada peça, contendo as medições dimensionais críticas realizadas.',
      'd) O posto de trabalho limpo e organizado, com as ferramentas e instrumentos devidamente guardados e os resíduos segregados corretamente.'
    ]
  }
];
